...
`--auto-wait-s N` bounds that wait; `--auto-wait-s 0` skips it and connects with whatever analysis is already stored (which may be incomplete). Use this when a database's analysis queue never drains. `--wait-s` is separate: it is the supervisor's timeout for seeing the client on the bridge.
